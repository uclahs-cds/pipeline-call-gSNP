...
process run_IndelRealigner_GATK {
    ...
    path(bundle_known_indels_vcf_gz)
    path(bundle_known_indels_vcf_gz_tbi)
    tuple val(sample_id), val(normal_id), val(tumour_id), path(bam), path(bam_index), path(bam_tumour), path(bam_index_tumour), path(interval), path(target_intervals_RTC)
    ...
    tuple val(sample_id),
          ...
          path(bam_index_tumour),
          path(interval),
          path("${sample_id}_indelrealigned_${task.index}.bam"),
          path("${sample_id}_indelrealigned_${task.index}.bai"), emit: realigned_indels_bam
